...
class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def configure_account_update_params
    ...
    devise_parameter_sanitizer.permit(:account_update, keys: [:friendly_id])
  ...
  def response_to_sign_up_failure(resource)
    if resource.email == "" && resource.password == nil
      redirect_to new_user_session_path, notice: "入力されていないフォームがあります"
    elsif User.pluck(:email).include? resource.email
      redirect_to new_user_session_path, notice: "そのメールアドレスは既に登録されています"
    end
  end
